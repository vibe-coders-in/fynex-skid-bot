    ...
    run: async (client, message, args) => {
        ...
        if(!data){
            ...
            const initialMessage = await message.channel.send({
                embeds: [new MessageEmbed().setColor(client.color).setDescription('<a:Tick:1306038825054896209>Configuring your server...')],
              });
              ...
              initialMessage.edit({
                embeds: [
                  new MessageEmbed()
                    .setColor(client.color)
                    .setTitle('<a:Tick:1306038825054896209>Server Configuration Successful')
                    .setDescription('<a:Tick:1306038825054896209>Your server has been successfully configured for logging.')
                    .setFooter({ text: '<a:Tick:1306038825054896209>Your server has been successfully configured for logging Run Command Again.!'})
                ],
              });
                      } 
            ...
            return message.channel.send({
                embeds: [
                    new MessageEmbed()
                        ...
                        .setFooter(`Note : If you wannna setup logging again use ${message.guild.prefix}logsreset & delete all existinglog channels of Flixo-Advance`, client.user.displayAvatarURL())
                ]
            });
        ...
        try {
            let category = message.guild.channels.cache.find(c => c.type === 'GUILD_CATEGORY' && c.name === 'Flixo-Advance-LOGS');

            if (!category) {
                category = await message.guild.channels.create('Flixo-Advance-LOGS', {
                    type: 'GUILD_CATEGORY',
    ...
    return message.channel.send({
        embeds: [
            new MessageEmbed()
                ...
                .setDescription('All necessary logging channels have been successfully created under the "Flixo-Advance LOGS" category.')
