            ...
            return message.channel.send({
                embeds: [
                    new MessageEmbed()
                        .setColor('BLACK')
                        .setDescription(
                            `<:anxCross:1317554876712222794> | You must mention a user or provide their ID to chatunban.`
                        ),
                ],
            });
        }
        ...
        if (member.id === message.guild.ownerId || member.id === client.user.id) {
            return message.channel.send({
                embeds: [
                    new MessageEmbed()
                        .setColor('BLACK')
                        .setDescription(`<:anxCross:1317554876712222794> | You cannot chatunban this user.`),
                ],
            });
        }
            ...
            message.channel.send({
                embeds: [
                    new MessageEmbed()
                        .setColor('GREEN')
                        .setDescription(
                            `<a:tk:1304818061034917979> | Successfully chatunbanned <@${member.user.id}>.`
                        ),
                ],
            });
        } catch (err) {
            ...
            message.channel.send({
                embeds: [
                    new MessageEmbed()
                        .setColor('BLACK')
                        .setDescription(
                            `<:anxCross:1317554876712222794> | An error occurred while trying to chatunban <@${member.user.id}>.`
                        ),
                ],
            });
        }
