...
const { executor, target, createdTimestamp } = logs;
  ...
  if(data) {
   ...
   channeldelete.addFields(
    ...
    { name: `Channel Mention`, value: `<#${channel.id}>` },
    { name: `Channel Id`, value: `${channel.id}` },
    { name: `Removed By`, value: `${executor}\n` },
    { name: `User Id`, value: `${executor.id}` },
  ...
  if(data){
    ...
    if (oldName !== newName) {
      channelupdate.addField('Channel Renamed', `\`${o.name}\` -> \`${n.name}\``, false);
  }
  
  if (oldNsfw !== newNsfw) {
      channelupdate.addField('Channel NSFW State Updated', `\`${o.nsfw}\` -> \`${n.nsfw}\``, false);
  }
  
  if (oldcooldown !== newcooldown) {
      channelupdate.addField('Channel Slowmode Updated', `\`${o.rateLimitPerUser} seconds\` -> \`${n.rateLimitPerUser} seconds\``, false);
  }
  
  if (oldTopic !== newTopic) {
      channelupdate.addField('Channel Topic Updated', `\`${o.topic}\` -> \`${n.topic}\``, false);
  }
  
  if (oldrtcRegion !== newrtcRegion) {
      channelupdate.addField('Channel Voice Region Updated', `\`${o.rtcRegion}\` -> \`${n.rtcRegion}\``, false);
  }
  
  if (oldbitrate !== newbitrate) {
      channelupdate.addField('Channel Voice Bitrate Updated', `\`${oldbitratevalue} Kbps\` -> \`${newbitratevalue} Kbps\``, false);
  }
  
  if (olduserLimit !== newuserLimit) {
      channelupdate.addField('Channel Voice User Limit Updated', `\`${o.userLimit} users\` -> \`${n.userLimit} users\``, false);
  } else if (o.type === 'GUILD_VOICE') {
      channelupdate.addField('Channel Voice Updated', 'Channel Video Quality Mode Updated', false);
  }
  
  channelupdate.addField('Updated At', `<t:${Math.round(logs.createdTimestamp / 1000)}:R>`, false);
